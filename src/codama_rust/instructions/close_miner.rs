...
pub const CLOSE_MINER_DISCRIMINATOR: [u8; 8] = [101, 10, 186, 29, 231, 198, 255, 67];

/// Accounts.
#[derive(Debug)]
pub struct CloseMiner {
...
impl CloseMiner {
    ...
    pub fn instruction_with_remaining_accounts(
        ...
        let data = CloseMinerInstructionData::new().try_to_vec().unwrap();
...
pub struct CloseMinerInstructionData {
    discriminator: [u8; 8],
}

impl CloseMinerInstructionData {
    pub fn new() -> Self {
        Self {
            discriminator: [101, 10, 186, 29, 231, 198, 255, 67],
        }
    }
...
impl Default for CloseMinerInstructionData {
    fn default() -> Self {
        Self::new()
    }
}

/// Instruction builder for `CloseMiner`.
///
/// ### Accounts:
///
...
/// `close_miner` CPI accounts.
pub struct CloseMinerCpiAccounts<'a, 'b> {
    /// Crank signer authorized to submit cleanup transactions.
...
/// `close_miner` CPI instruction.
pub struct CloseMinerCpi<'a, 'b> {
...
impl<'a, 'b> CloseMinerCpi<'a, 'b> {
    pub fn new(
        program: &'b solana_account_info::AccountInfo<'a>,
        accounts: CloseMinerCpiAccounts<'a, 'b>,
    ) -> Self {
        Self {
            __program: program,
...
/// Instruction builder for `CloseMiner` via CPI.
///
/// ### Accounts:
///
///   0. `[writable, signer]` signer
...
///   4. `[writable]` player
#[derive(Clone, Debug)]
pub struct CloseMinerCpiBuilder<'a, 'b> {
    instruction: Box<CloseMinerCpiBuilderInstruction<'a, 'b>>,
}

impl<'a, 'b> CloseMinerCpiBuilder<'a, 'b> {
    pub fn new(program: &'b solana_account_info::AccountInfo<'a>) -> Self {
        let instruction = Box::new(CloseMinerCpiBuilderInstruction {
            __program: program,
            signer: None,
            config: None,
            round: None,
...
#[derive(Clone, Debug)]
struct CloseMinerCpiBuilderInstruction<'a, 'b> {
